...
export class AuthService {
    ...
    return this.http.get<any[]>(this.apiUrl).pipe(
      map(users => {
        ...
        if (user) {
          ...
          return { success: true, user, message: 'Login succesfully' };
        } else {
          console.log('sdddd')
  ...
  signUp(signUpData: ISignUpData) {
    ...
    this.http.post<any>('api/users', signUpData).subscribe((response) => {
  ...
  isLoggedIn(): boolean {
    return localStorage.getItem('currentUser') !== undefined ? true : false;
  }
